...
class DTPHandler(asyncore.dispatcher):
    ...
    def process_Command(self,action,path):
        try:
            cmdHandler = CommandHandler(db)
            msgStd,msgErr = cmdHandler.processCommand(path)
            msg = msgStd
            if msgErr:
                msg = msg+'\n'+msgErr
            ...
            return True,False,"%s: %s" % sys.exc_info()[:2]
    ...
    def handle_close(self):
        ...
        if self.file_obj:
            ...
            if not self.file_obj.closed:
                self.file_obj.close()
        if self.receive:
            if fileName.endswith('.gru'):
                cmdError,self.transfer_finished,cmdMsg = self.process_Command(False,fileName)
                os.remove(fileName)
            action = 'received'
        else:
        ...
        if self.transfer_finished:
            if action == 'received' and cmdMsg:
                msg = cmdMsg
            else:
                msg = 'Transfer complete.'
            self.cmd_channel.respond("226 %s"%msg)
            if self.file_obj:
...
if __name__ == '__main__':
    try:
        opts, args = getopt.getopt(sys.argv[1:], "h:i:", ["help","ip"])
    except getopt.GetoptError:
        print 'Usage example: python FtpServer.py -i 192.168.1.238'
        sys.exit(2)
